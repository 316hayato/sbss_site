class Admin::RequestDetailsController < ApplicationController
  ...
  storage_status_address = params[:request_detail][:storage_status]         # 保存ステータス
  sale_status_address = params[:request_detail][:sale_status]         # 売却ステータス
  disposal_status_address = params[:request_detail][:disposal_status] # 処理ステータス
    # 複数のステータスが「完了」している場合
